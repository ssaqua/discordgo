...
func TestRatelimitReset(t *testing.T) {
	...
	sendReq := func(endpoint string) {
		...
		headers.Set("X-RateLimit-Reset", strconv.FormatInt(time.Now().Add(time.Second*2).Unix(), 10) + ".000")
...
func sendBenchReq(endpoint string, rl *RateLimiter) {
	...
	headers.Set("X-RateLimit-Reset", strconv.FormatInt(time.Now().Unix(), 10) + ".000")
	headers.Set("Date", time.Now().Format(time.RFC850))
